...
namespace TownOfUs.CultistRoles.WhispererMod
{
    ...
    public class HudManagerUpdate
    {
        ...
        public static void Postfix(HudManager __instance)
        {
            ...
            __instance.AbilityButton.SetCoolDown(role.WhisperTimer(),
                CustomGameOptions.WhisperCooldown + CustomGameOptions.IncreasedCooldownPerWhisper * role.WhisperCount);

            var renderer = __instance.AbilityButton.graphic;
            if (!__instance.AbilityButton.isCoolingDown && __instance.AbilityButton.gameObject.active)
            {
